...
def test_random_planner_uses_the_supplied_macro_goal_once_and_keeps_seven_days(monkeypatch):
    monkeypatch.setattr(
        planner,
        "generate_daily_macro_goal",
        lambda _preference: (_ for _ in ()).throw(AssertionError("must not generate macros twice")),
    )
    monkeypatch.setattr(planner, "build_recipe_pool", lambda *_args, **_kwargs: {})
    monkeypatch.setattr(planner, "_pick_day_combo", lambda *_args: ([], 0))
    monkeypatch.setattr(planner, "apply_carry_forward_leftovers", lambda plan, *_args, **_kwargs: plan)
    ...
    result = planner.generate_daily_plan(preference, macro_goal=macro_goal, db=object())
