...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint(
            "instagram_username", name="uq_recipe_creators_username"
        ),
    )
    op.create_table(
        "recipe_import_jobs",
        ...
        sa.Column(
            "creator_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("recipe_creators.id"),
            nullable=False,
        ),
    ...
    op.create_index(
        "ix_recipe_import_jobs_creator_id", "recipe_import_jobs", ["creator_id"]
    )
    op.create_index(
        "ix_recipe_import_jobs_claim",
        "recipe_import_jobs",
        ["status", "created_at"],
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column(
            "job_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("recipe_import_jobs.id"),
            nullable=False,
        ),
        ...
        sa.UniqueConstraint(
            "creator_id",
            "platform_post_id",
            name="uq_recipe_source_posts_external",
        ),
    )
    op.create_index(
        "ix_recipe_source_posts_job", "recipe_source_posts", ["job_id"]
    )
    op.create_table(
        "recipe_candidates",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column(
            "source_post_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("recipe_source_posts.id"),
            nullable=False,
        ),
        sa.Column(
            "creator_id",
    ...
    op.create_table(
        ...
        sa.Column(
            "candidate_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("recipe_candidates.id"),
            nullable=False,
        ),
        ...
        sa.UniqueConstraint(
            "candidate_id",
            "version",
            name="uq_recipe_candidate_extraction_version",
        ),
    )
    op.create_table(
    ...
    op.execute(
        "UPDATE recipes SET source_kind = 'legacy_matprat' "
