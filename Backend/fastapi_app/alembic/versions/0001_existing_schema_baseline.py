...
Fresh databases can run this revision normally. For an existing, unversioned
database, first take and verify a restore point, inspect that its schema is
compatible, then run ``alembic stamp 0001_existing_schema`` followed by
``alembic upgrade head``. The guard below intentionally rejects applying this
DDL to an existing schema.
"""
...
class VectorType(sa.types.UserDefinedType):
    """Render pgvector without coupling this historical revision to runtime models."""
...
def upgrade() -> None:
    if not context.is_offline_mode():
        ...
        if inspector.has_table("recipes"):
            raise RuntimeError(
                "Existing schema detected. Take and verify a restore point, "
                "inspect compatibility, then run `alembic stamp "
                "0001_existing_schema` followed by `alembic upgrade head`."
            )
    ...
    op.create_table(
        ...
        sa.Column("is_active", sa.Boolean(), nullable=False, server_default="true"),
        sa.CheckConstraint("(cost_category IS NULL) OR (cost_category IN ('cheap','medium expensive'))", name="recipes_cost_category_check"),
    )
    op.create_table(
        "activity_logs",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
    ...
    op.create_table(
        "plan_recipes",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("preference_id", sa.Integer(), sa.ForeignKey("preferences.id"), nullable=False),
        sa.Column("recipe_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("recipes.id"), nullable=False),
        sa.Column("day_name", sa.String(16)),
        sa.Column("meal_type", sa.String(16)),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
    op.create_index("ix_plan_recipes_preference_id", "plan_recipes", ["preference_id"])


def downgrade() -> None:
    for table in ("plan_recipes", "ratings", "preferences", "activity_logs", "recipes", "users"):
        op.drop_table(table)
